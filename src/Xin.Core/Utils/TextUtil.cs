...
namespace Xin.Core.Utils
{
    ...
    public class TextUtil
    {
        ...
        public static byte[] Encode7Bit(string sourceString)
        {
            ...
            var dataLength = sourceBytes.Length * 7;
            dataLength = dataLength % 8 == 0 ? dataLength / 8 : dataLength / 8 + 1;
            var destBytes = new byte[dataLength];
            var byteIndex1 = 0;
            var bitIndex1 = 0;
            var byteIndex2 = 0;
            var bitIndex2 = 0;
            ...
            while (byteIndex1 < destBytes.Length)
            {
                ...
                if (bitIndex2 > 7)
                {
                    ...
                    bitIndex2 = n - 8 * byteIndex2;
                    destBytes[byteIndex1] |= (byte) (sourceBytes[index] << bitIndex1);
                    destBytes[byteIndex2] |= (byte) (sourceBytes[index] >> (8 - bitIndex1));
                }
                else
                {
                    destBytes[byteIndex1] |= (byte) (sourceBytes[index] << bitIndex1);
        ...
        {
            var destBytesLength = sourceBytes.Length * 8 / 7;
            var destBytes = new byte[destBytesLength];
            var byteIndex1 = 0;
            var bitIndex1 = 0;
            var byteIndex2 = 0;
            var bitIndex2 = 0;
            ...
            while (byteIndex1 < sourceBytes.Length)
            {
                ...
                if (bitIndex2 > 7)
                {
                    ...
                    bitIndex2 = n - 8 * byteIndex2;
                    destBytes[index] = (byte) (sourceBytes[byteIndex1] >> bitIndex1);
                    destBytes[index] |= (byte) (sourceBytes[byteIndex2] << (8 - bitIndex1));
                }
                else
                {
                    destBytes[index] = (byte) (sourceBytes[byteIndex1] >> bitIndex1);
        ...
        private const string CODE_DATA =
        ...
        {
            var gb2312Bytes = Encoding.GetEncoding("GB2312").GetBytes(new[] {_char});
            var gb2312Code = Convert.ToInt32(string.Format("{0:D2}{1:D2}", gb2312Bytes[0] - 160, gb2312Bytes[1] - 160));
            ...
            {
                var position = (gb2312Bytes[0] - 160 - 56) * 94 + gb2312Bytes[1] - 160;
